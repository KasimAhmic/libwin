...
Napi::Value User32::MoveWindow(const Napi::CallbackInfo &info) {
  const Napi::Env env = info.Env();

  const QB_ARG(hWnd, qb::ReadRequiredHandle<HWND>(info, 0));
  const QB_ARG(X, qb::ReadRequiredInt32(info, 1));
  const QB_ARG(Y, qb::ReadRequiredInt32(info, 2));
  const QB_ARG(nWidth, qb::ReadRequiredInt32(info, 3));
  const QB_ARG(nHeight, qb::ReadRequiredInt32(info, 4));
